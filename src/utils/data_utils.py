...
def revert_output_format(poses, data_mean, data_std, dim_to_ignore, actions):
    ...
    for i in xrange(poses_out.shape[0]):
        poses_out_list.append(unNormalizeData(
            poses_out[i, :, :], data_mean, data_std, dim_to_ignore, actions))
    ...
    for subj in subjects:
        for action_idx in np.arange(len(actions)):
            ...
            for subact in [1, 2]:  # subactions
                logging.info("Reading subject {0}, action {1}, subaction {2}".format(
                    subj, action, subact))
...
def read_all_data(actions, seq_length_in, seq_length_out, data_dir):
    ...
    train_subject_ids = [1, 5, 6, 7, 9, 11]
    test_subject_ids = [8]
    ...
    train_set = normalize_data(
        train_set, data_mean, data_std, dim_to_use, actions)
    test_set = normalize_data(test_set,  data_mean,
                              data_std, dim_to_use, actions)
